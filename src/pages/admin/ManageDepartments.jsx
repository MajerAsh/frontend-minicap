...
const DepartmentForm = ({
  ...
  const facultyNotInDept = faculty.filter(
        ...
        <div className="faculty-list-box">
          ...
          {facultyInDept.map((prof) => (
            <div key={prof.id}>
              ...
              <button
                type="button"
                onClick={() => onFacultyChange(prof, null)}
                disabled={loading}
              >
                Remove
              </button>
        ...
        <div className="faculty-list-box">
          ...
          {facultyNotInDept.map((prof) => (
            <div key={prof.id}>
              ...
              <button
                type="button"
                onClick={() => onFacultyChange(prof, department.id)}
                disabled={loading}
              >
                Add
              </button>
...
export default function ManageDepartments() {
  ...
  const handleFormSubmit = async (formData) => {
    try {
      setLoading(true);
      const action =
        editingDept && editingDept.id ? updateDepartment : createDepartment;
      const data =
        editingDept && editingDept.id
          ? [editingDept.id, formData, token]
          : [formData, token];

      await action(...data);
      alert(
        `Department ${
          editingDept && editingDept.id ? "updated" : "created"
        } successfully!`
      );
  ...
  return (
    <div>
      ...
      {!editingDept && (
        <button onClick={handleAddClick}>Add New Department</button>
      )}
      {editingDept && !editingDept.id && (
        <DepartmentForm
          initialData={{}}
